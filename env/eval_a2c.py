...
print("✅ Loaded A2C model from", MODEL_PATH)
...
# ==== Save to CSV ====
a2c_ev_df = pd.DataFrame(ev_data)
a2c_ev_df.to_csv("a2c_eval_results.csv", index=False)
print("📁 A2C EV results saved to a2c_eval_results.csv (ev_id, wait_time, travel_time)")
...
plt.title("EV Waiting Time Over Time - A2C Controller")
plt.xlabel("Timestep")
...
plt.title("Queue Length Over Time - A2C Controller")
plt.xlabel("Timestep")
...
plt.title("Reward Over Time - A2C Controller")
plt.xlabel("Timestep")
plt.ylabel("Reward")
plt.grid(True)
plt.legend()
